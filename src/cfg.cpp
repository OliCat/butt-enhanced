...
int cfg_write_file(char *path)
{
    ...
    // AES67 section
    fprintf(cfg_fd, "[aes67]\n");
    fprintf(cfg_fd, "active = %d\n", cfg.aes67.active);
    fprintf(cfg_fd, "ip = %s\n", cfg.aes67.ip ? cfg.aes67.ip : "239.69.145.58");
    fprintf(cfg_fd, "port = %d\n", cfg.aes67.port);
    fprintf(cfg_fd, "ttl = %d\n", cfg.aes67.ttl);
    fprintf(cfg_fd, "dscp = %d\n", cfg.aes67.dscp);
    fprintf(cfg_fd, "iface = %s\n", cfg.aes67.iface ? cfg.aes67.iface : "");
    fprintf(cfg_fd, "loopback = %d\n", cfg.aes67.loopback);
    fprintf(cfg_fd, "ptp = %d\n", cfg.aes67.ptp);
    fprintf(cfg_fd, "sap = %d\n\n", cfg.aes67.sap);
    ...
    fprintf(cfg_fd,
            ...
            "replace_dsp = %d\n"
            "license_key = \"%s\"\n"
            "preset_file_stream = \"%s\"\n"
            "preset_file_rec = \"%s\"\n"
            "latency_compensation = %d\n"
            "bypass_on_silence = %d\n"
            "bypass_threshold = %f\n\n",
            cfg.stereo_tool.enabled_stream,
            cfg.stereo_tool.enabled_rec,
            cfg.stereo_tool.replace_dsp,
            cfg.stereo_tool.license_key != NULL ? cfg.stereo_tool.license_key : "",
            cfg.stereo_tool.preset_file_stream != NULL ? cfg.stereo_tool.preset_file_stream : "",
            cfg.stereo_tool.preset_file_rec != NULL ? cfg.stereo_tool.preset_file_rec : "",
            cfg.stereo_tool.latency_compensation,
            cfg.stereo_tool.bypass_on_silence,
            cfg.stereo_tool.bypass_threshold);
...
int cfg_set_values(char *path)
{
    ...
    // AES67 defaults + parsing
    cfg.aes67.active = cfg_get_int("aes67", "active", 0);  // Default: disabled
    cfg.aes67.ip = cfg_get_str("aes67", "ip", (char*)"239.69.145.58");
    cfg.aes67.port = cfg_get_int("aes67", "port", 5004);
    cfg.aes67.ttl = cfg_get_int("aes67", "ttl", 32);
    cfg.aes67.dscp = cfg_get_int("aes67", "dscp", 46);
    cfg.aes67.iface = cfg_get_str("aes67", "iface", (char*)"");
    cfg.aes67.loopback = cfg_get_int("aes67", "loopback", 0);
    cfg.aes67.ptp = cfg_get_int("aes67", "ptp", 0);
    cfg.aes67.sap = cfg_get_int("aes67", "sap", 0);

    // Audio performance options
    cfg.audio_perf.use_vdsp = cfg_get_int("audio_perf", "use_vdsp", 1);
    cfg.audio_perf.dither_type = cfg_get_int("audio_perf", "dither_type", 1); // TPDF par défaut
    cfg.audio_perf.pll_enabled = cfg_get_int("audio_perf", "pll_enabled", 0);
    cfg.audio_perf.pll_window_s = cfg_get_float("audio_perf", "pll_window_s", 2.0);
    cfg.audio_perf.clip_protection = cfg_get_int("audio_perf", "clip_protection", 1);
    ...
    // read stereo tool related stuff
    cfg.stereo_tool.enabled_stream = cfg_get_int("stereo_tool", "enabled_stream", 0);
    cfg.stereo_tool.enabled_rec = cfg_get_int("stereo_tool", "enabled_rec", 0);
    cfg.stereo_tool.replace_dsp = cfg_get_int("stereo_tool", "replace_dsp", 1);
    
    // Read and clean license key (remove quotes)
    char *raw_license_key = cfg_get_str("stereo_tool", "license_key", NULL);
    if (raw_license_key != NULL) {
        // Remove quotes from beginning and end
        char *cleaned_key = raw_license_key;
        size_t len = strlen(raw_license_key);
        if (len > 0 && raw_license_key[0] == '"') {
            cleaned_key++;
            len--;
        }
        if (len > 0 && cleaned_key[len-1] == '"') {
            cleaned_key[len-1] = '\0';
        }
        cfg.stereo_tool.license_key = strdup(cleaned_key);
        free(raw_license_key);
    } else {
        cfg.stereo_tool.license_key = NULL;
    }
    
    // Read and clean preset file paths (remove quotes)
    char *raw_preset_stream = cfg_get_str("stereo_tool", "preset_file_stream", NULL);
    if (raw_preset_stream != NULL) {
        char *cleaned_preset = raw_preset_stream;
        size_t len = strlen(raw_preset_stream);
        if (len > 0 && raw_preset_stream[0] == '"') {
            cleaned_preset++;
            len--;
        }
        if (len > 0 && cleaned_preset[len-1] == '"') {
            cleaned_preset[len-1] = '\0';
        }
        cfg.stereo_tool.preset_file_stream = strdup(cleaned_preset);
        free(raw_preset_stream);
    } else {
        cfg.stereo_tool.preset_file_stream = NULL;
    }
    
    char *raw_preset_rec = cfg_get_str("stereo_tool", "preset_file_rec", NULL);
    if (raw_preset_rec != NULL) {
        char *cleaned_preset = raw_preset_rec;
        size_t len = strlen(raw_preset_rec);
        if (len > 0 && raw_preset_rec[0] == '"') {
            cleaned_preset++;
            len--;
        }
        if (len > 0 && cleaned_preset[len-1] == '"') {
        ...
        cfg.stereo_tool.preset_file_rec = strdup(cleaned_preset);
        free(raw_preset_rec);
    } else {
        cfg.stereo_tool.preset_file_rec = NULL;
    }
    cfg.stereo_tool.latency_compensation = cfg_get_int("stereo_tool", "latency_compensation", 1);
    cfg.stereo_tool.bypass_on_silence = cfg_get_int("stereo_tool", "bypass_on_silence", 0);
    cfg.stereo_tool.bypass_threshold = cfg_get_float("stereo_tool", "bypass_threshold", -40.0);
