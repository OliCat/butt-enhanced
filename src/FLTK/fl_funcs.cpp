...
void fill_cfg_widgets(void)
{
    ...
    // fill StereoTool section
    fl_g->check_stereo_tool_stream->value(cfg.stereo_tool.enabled_stream);
    fl_g->check_stereo_tool_record->value(cfg.stereo_tool.enabled_rec);
    fl_g->check_stereo_tool_replace_dsp->value(cfg.stereo_tool.replace_dsp);
    fl_g->input_stereo_tool_license->value(cfg.stereo_tool.license_key);
    ...
    if (cfg.stereo_tool.preset_file_stream) {
        char *basename = strrchr(cfg.stereo_tool.preset_file_stream, '/');
        if (basename) basename++;
        else basename = cfg.stereo_tool.preset_file_stream;
    ...
    if (cfg.stereo_tool.preset_file_rec) {
        char *basename = strrchr(cfg.stereo_tool.preset_file_rec, '/');
        if (basename) basename++;
        else basename = cfg.stereo_tool.preset_file_rec;
        fl_g->choice_stereo_tool_preset_record->add(basename);
        fl_g->choice_stereo_tool_preset_record->value(0);
    }
    
    // Initialize StereoTool library at startup for better UI responsiveness
...
void update_codec_samplerates(void)
{
    ...
    // Update StereoTool instances if enabled
...
{
    // Variable externe pour vérifier les signaux de fermeture
    ...
    if (is_bundle) {
        printf("BUTT: MODE BUNDLE macOS - vérification accélérée des signaux\n");
    }
#endif
    ...
    while (!g_shutdown_requested) {
        ...
        // Vérification périodique des signaux (plus fréquente pour les bundles)
#ifdef __APPLE__
        if (is_bundle) {
            // Pour les bundles, vérifier très fréquemment
            if (iteration_count % 10 == 0) {
                if (g_shutdown_requested) {
                    printf("BUTT: Signal de fermeture détecté (bundle mode)\n");
                    break;
                }
            }
            // Pause très courte pour les bundles
            usleep(1000); // 1ms
        } else {
            // Pour les exécutables normaux
            if (events == 0 && !g_shutdown_requested) {
                usleep(10000); // 10ms
            }
        }
#else
        // Si plus d'événements et pas de signal, faire une petite pause
        if (events == 0 && !g_shutdown_requested) {
            usleep(10000); // 10ms
        }
#endif
        
        // Vérifier si un signal de fermeture a été reçu
        if (g_shutdown_requested) {
            printf("BUTT: Signal de fermeture détecté, arrêt de la boucle GUI (itération %d)\n", iteration_count);
            break;
        }
        
        // Limite de sécurité pour éviter les boucles infinies
        if (iteration_count > 1000000) {
            printf("BUTT: Limite d'itérations atteinte, vérification forcée\n");
            iteration_count = 0;
        }
    }
    
    printf("BUTT: Sortie de la boucle GUI après %d itérations\n", iteration_count);
    return 0;
}
